...
- 对应技术资料：[14-深度学习框架与计算图](../../01-技术资料/07-AI-Agent全栈开发/14-深度学习框架与计算图.md)
...
| #   | 考点               | 核心要点（一句话）                                   | 出题概率 |
| --- | ------------------ | ---------------------------------------------------- | -------- |
| 1   | 计算图             | 静态(TF)定义后执行 vs 动态(PyTorch)即时构建          | ★★★★★    |
| 2   | 自动微分           | 前向模式(Jacobian列) vs 反向模式(行)，深度学习用反向 | ★★★★★    |
| 3   | PyTorch Autograd   | grad_fn 链表 + backward() 反向传播                   | ★★★★☆    |
| 4   | torch.compile      | TorchDynamo字节码分析 + TorchInductor 代码生成       | ★★★★☆    |
| 5   | 分布式训练 DDP     | All-Reduce梯度同步，Bucket机制重叠通信与计算         | ★★★★☆    |
| 6   | FSDP vs DDP        | FSDP 分片参数(类ZeRO-3)，DDP 复制参数                | ★★★☆☆    |
| 7   | 混合精度训练       | FP16前向+FP32优化器，Loss Scaling 防下溢             | ★★★★☆    |
| 8   | TF vs PyTorch 选型 | PyTorch=研究/LLM，TF=移动端/TPU/TFX                  | ★★★★☆    |
| 9   | tf.function        | 追踪→图→优化，input_signature 处理动态形状           | ★★★☆☆    |
| 10  | 梯度消失/爆炸      | 深层网络链式乘积，解法: BatchNorm/ResNet/梯度裁剪    | ★★★★☆    |
