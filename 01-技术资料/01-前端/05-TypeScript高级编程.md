...
// 使用默认值
const res1: ApiResponse = { data: null, error: new Error('fail'), statusCode: 500 }
// 覆盖默认值
const res2: ApiResponse<User> = { data: { name: 'Alice', email: '' }, error: null, statusCode: 200 }
```
...
const newUser = await request('/users', 'POST', { body: { name: 'Bob', email: 'b@b.com' } })
// newUser 的类型是 User ✅
```
...
// 有 const：推断为字面量元组
function routesConst<const T extends readonly string[]>(paths: T): T { return paths }
...
```typescript
// ❌ 1. any 滥用：关闭类型检查
function parse(data: any) { return data.name }  // 运行时可能崩溃
// ✅ 用 unknown + 收窄
...
// ❌ 3. enum 过度使用
enum Status { Active, Inactive }  // 运行时有开销
// ✅ 用联合类型替代
