...
定义：
  countAtom = 5
  doubleCountAtom = get(countAtom) * 2 → 10
  tripleCountAtom = get(countAtom) * 3 → 15
  sumAtom = get(doubleCountAtom) + get(tripleCountAtom) → 25
...
当 countAtom 更新为 6：
  1. 标记 doubleCountAtom、tripleCountAtom 为脏（dirty）
  2. 重新计算 doubleCountAtom = 12
  3. 重新计算 tripleCountAtom = 18
  4. 重新计算 sumAtom = 30
  5. 通知订阅了这些原子的组件 re-render
```
