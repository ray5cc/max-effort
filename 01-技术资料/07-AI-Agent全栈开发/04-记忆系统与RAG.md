...
<!-- ORIGINAL: RAG 全链路 Loading→Chunking→Indexing→Storing→Querying→Evaluation 流程图 -->
```mermaid
graph TD
    subgraph indexing["离线索引"]
        A["Loading / 文档加载"] --> B["Chunking / 分块切割"] --> C["Indexing / 向量化"] --> D["Storing / 持久化"]
    end
    D --> E
    subgraph querying["Querying 查询"]
        E["用户 Query"] --> F["Query 向量化"] --> G["ANN 搜索（HNSW/IVF）"] --> H["Re-ranking（可选）"] --> I["上下文注入 → LLM 生成"]
    end
    I --> J["Evaluation 评估<br/>RAGAS: Faithfulness / Answer Relevancy /<br/>Context Precision / Context Recall"]
```
...
<!-- ORIGINAL: Self-RAG 流程图：Retrieve token 预测分支 → IsRel/IsSup/IsUse → 最终答案 -->
```mermaid
graph TD
    A["用户 Query"] --> B{"Retrieve token 预测"}
    B -->|"Retrieve=Yes"| C["触发检索"]
    C --> D["IsRel token（是否相关）"]
    C --> E["IsSup token（是否支撑答案）"]
    B -->|"Retrieve=No"| F["直接生成答案"]
    D --> G["IsUse token 预测（答案是否有用）"]
    E --> G
    F --> G
    G --> H["选择最优生成路径"] --> I["最终答案"]
```
...
<!-- ORIGINAL: CRAG 流程图：Query→检索→评估器→三级置信度分支→生成答案 -->
```mermaid
graph LR
    A["Query"] --> B["检索"] --> C{"评估器"}
    C -->|"置信度高"| D["直接使用文档"] --> G["生成答案"]
    C -->|"置信度中"| E["知识精炼（去噪）"] --> G
    C -->|"置信度低"| F["Web 搜索补充"] --> H["知识精炼"] --> G
```
...
<!-- ORIGINAL: RAGAS 评估体系：question→context/answer/ground_truth→四大指标 -->
```mermaid
graph TD
    Q["question"] --> Ctx["context"]
    Q --> Ans["answer"]
    Q --> GT["ground_truth"]
    Ans --> F["Faithfulness<br/>答案是否由上下文支撑（不幻觉）"]
    Ctx --> CP["Context Precision<br/>检索到的上下文中有多少是真正有用的"]
    GT --> AR["Answer Relevancy<br/>答案与问题的相关性（语义相似度）"]
    CP --> CR["Context Recall<br/>标准答案中的信息有多少被检索到"]
```
...
<!-- ORIGINAL: 混合检索架构（Hybrid Search）：BM25+Dense双路→Score Fusion→Top-K -->
```mermaid
graph TD
    Q["用户 Query"]
    Q --> BM["BM25 稀疏检索管道<br/>倒排索引 / 关键词匹配"]
    Q --> Dense["Dense 向量检索管道<br/>HNSW/IVF / 语义相似度"]
    BM -->|"Top-K₁"| Fusion["Score Fusion（加权 / RRF）"]
    Dense -->|"Top-K₂"| Fusion
    Fusion --> Result["Top-K 最终结果"]
```
...
<!-- ORIGINAL: 三路融合检索架构：向量+BM25+知识图谱→RRF→Cross-Encoder→LLM -->
```mermaid
graph TD
    Q["用户 Query"]
    Q --> V["向量检索<br/>Milvus/Qdrant<br/>语义相似度"]
    Q --> BM["BM25 检索<br/>ES/OpenSearch<br/>精确关键词"]
    Q --> KG["知识图谱检索<br/>Neo4j 图遍历 + Cypher 查询<br/>结构化关系"]
    V -->|"Top-50"| RRF["RRF 融合排序"]
    BM -->|"Top-50"| RRF
    KG -->|"Top-20"| RRF
    RRF --> Cand["Top-100 候选"]
    Cand --> CE["Cross-Encoder 重排序（精排）"]
    CE --> Final["Top-10 → LLM 生成"]
```
...
<!-- ORIGINAL: Graph RAG 完整流水线：离线（分块→实体/关系抽取→图谱→社区检测）+ 在线（查询→局部/全局搜索→LLM） -->
```mermaid
graph TD
    subgraph offline["离线阶段（Indexing Pipeline）"]
        A["原始文档"] --> B["文本分块 Chunking"]
        B --> C["实体抽取 NER/LLM"]
        C --> D["关系抽取 RE/LLM"]
        D --> E["知识图谱构建<br/>实体去重/合并 → Neo4j 写入"]
        E --> F["社区检测（Leiden）"]
        E --> G["向量索引构建（嵌入实体描述）"]
        F --> H["社区摘要生成（LLM 总结）"]
    end
...
<!-- ORIGINAL: 生产级 Retrieve-Rerank Pipeline：Query预处理→多路召回→重排序→后处理→LLM生成 -->
```mermaid
graph TD
    Q["用户 Query"] --> A["Query 预处理<br/>改写 / 扩展 / HyDE"]
    A --> B["多路召回（粗筛）Top-100<br/>向量 + BM25 → RRF ~50ms"]
    B --> C["重排序（精排）Top-10<br/>Cross-Encoder / ColBERT ~200ms"]
    C --> D["后处理 Top-5<br/>去重 / 多样性过滤 / 上下文压缩"]
    D --> E["LLM 生成最终答案<br/>~1-3s"]
```
