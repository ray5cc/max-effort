...
<!-- ORIGINAL: 多 Agent 系统全景（Orchestrator、Agent Pool、Shared State、工具层） -->
```mermaid
graph TD
    Orch["Orchestrator（编排者）"] -->|"消息/事件"| Pool
    Pool -->|"结果/状态"| Orch
    Orch -->|"协调"| State["Shared State（共享状态）<br/>- 任务队列<br/>- 中间结果<br/>- 历史对话"]
    ...
    Tools["工具层 Tools<br/>搜索 | 代码执行 | 数据库 | API 调用"]
```
...
<!-- ORIGINAL: Reflection 模式（Generator ↔ Critic 迭代循环 → Final Output） -->
```mermaid
graph TD
    Gen["Generator（生成者）"] -->|"初稿"| Crit["Critic（批评者）"]
    Crit -->|"改进建议"| Gen
    Gen -->|"迭代 N 次后"| Final["Final Output"]
```
