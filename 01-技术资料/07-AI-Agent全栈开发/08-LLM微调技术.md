...
<!-- ORIGINAL: 技术选型决策树（问题类型 → 推荐方案映射） -->
```mermaid
graph LR
    A["动态知识（频繁更新）"] --> B["RAG（检索增强）"]
    C["风格/格式适配"] --> D["Prompt Engineering（零成本）"]
    E["领域知识固化"] --> F["SFT 微调"]
    G["行为对齐/安全"] --> H["RLHF / DPO 对齐"]
    I["资源极度受限"] --> J["Prompt Engineering / RAG"]
    K["数据保密（不上云）"] --> L["私有化微调 + 本地部署"]
```
...
<!-- ORIGINAL: Adapter 模块结构（降维→激活→升维→残差连接） -->
```mermaid
graph TD
    A["输入 h（维度 d）"] --> B["线性降维层（d → m，m ≪ d）"]
    B --> C["非线性激活（ReLU / GELU）"]
    C --> D["线性升维层（m → d）"]
    D -->|"残差连接"| E["输出 h + Adapter(h)"]
```
...
<!-- ORIGINAL: RLHF 四阶段流程（预训练→SFT→Reward Model→PPO优化） -->
```mermaid
graph TD
    A["阶段1: 预训练基础模型<br/>Base LLM — 在海量无监督文本上预训练"] --> B["阶段2: 监督微调（SFT）<br/>SFT Model — 人工标注的示例对话"]
    B --> C["阶段3: 训练 Reward Model（RM）<br/>对同一 Prompt 的多个回答进行人工排序<br/>学习'什么是好的回答'"]
    C --> D["阶段4: PPO 强化学习优化<br/>RL Model — PPO 算法最大化期望奖励<br/>使用 RM 打分 + KL 散度约束防止退化"]
```
...
<!-- ORIGINAL: DPO 两阶段流程（SFT→偏好优化，含 π_θ 与 π_ref 分支） -->
```mermaid
graph TD
    A["阶段1: SFT（监督微调）<br/>Prompt-Response 数据对训练<br/>此模型同时作为 π_ref（冻结）"] --> B["阶段2: DPO 偏好优化"]
    B --> C["偏好数据集<br/>{prompt, chosen, rejected}"]
    C --> D["π_θ（训练）"]
    C --> E["π_ref（冻结）"]
    D --> F["DPO Loss 计算<br/>（最大化偏好差距）"]
    E --> F
```
