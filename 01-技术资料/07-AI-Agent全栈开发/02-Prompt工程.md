...
<!-- ORIGINAL: AI Agent 全栈架构中 Prompt 工程层的位置 -->
```mermaid
graph TD
    A["用户请求"] --> B["Agent Orchestrator"]
    B --> C["Prompt 工程层 ◄ 本文档重点<br/>· 系统提示设计<br/>· CoT/ReAct<br/>· 输出格式约束"]
    C --> D["LLM（GPT/Claude/Gemini）"]
    D --> E["OutputParser 层<br/>结构化 → 业务对象"]
```
...
```mermaid
graph TD
    A{"任务是否需要外部工具？"} -->|"是"| B["ReAct"]
    A -->|"否"| C{"任务是否有唯一正确答案？"}
    C -->|"是 + 高精度要求"| D["Self-Consistency"]
    C -->|"是 + 中等精度"| E["CoT"]
    C -->|"否（创意/对话）"| F["Few-shot 或 Zero-shot"]
```
...
<!-- ORIGINAL: LCEL 管道类型流转图 -->
```mermaid
graph TD
    A["Dict[str, Any]"] -->|"prompt.invoke(dict) → PromptValue"| B["ChatPromptTemplate"]
    B -->|"llm.invoke(PromptValue) → AIMessage"| C["ChatOpenAI"]
    C -->|"parser.invoke(AIMessage) → T（目标类型）"| D["OutputParser"]
    D --> E["T（Pydantic 实例 / str / dict / ...）"]
```
...
```mermaid
graph TD
    A["1. 定义任务 + 评估指标"] --> B["2. 生成初始提示（人工或 LLM 生成）"]
    B --> C["3. 在评估集上运行 → 收集结果"]
    C --> D["4. 元提示 LLM 分析失败案例"]
    D --> E["5. 生成改进版提示"]
    E -->|"指标未达标"| C
    E -->|"指标达标"| F["完成优化"]
```
...
<!-- ORIGINAL: Prompt 管理流程（类比 Git + CI/CD） -->
```mermaid
graph LR
    A["修改 Prompt<br/>(commit)<br/>记录变更 + 变更理由"] --> B["评估测试集<br/>(CI test)<br/>回归检测 通过/失败"]
    B --> C["灰度发布<br/>(canary)<br/>5% 流量 A/B 对比"]
    C --> D["全量上线<br/>(deploy)<br/>确认无退化 切换 100%"]
```
