...
<!-- ORIGINAL: 工具调用完整流程（用户→应用层→LLM→工具调度器→工具执行→LLM最终回复） -->
```mermaid
sequenceDiagram
    participant User as 用户
    participant App as 应用层
    participant LLM as LLM（GPT-4/Claude/Gemini）
    participant Dispatcher as 工具调度器

    User->>App: 用户输入
    App->>LLM: 请求 + 工具定义
    LLM-->>App: tool_calls JSON 列表
    App->>Dispatcher: 解析 tool_calls
    Note over Dispatcher: 并行/顺序执行<br/>Tool A（search_web）→ 搜索结果<br/>Tool B（get_weather）→ 天气数据<br/>Tool C（run_code）→ 执行结果
    Dispatcher-->>App: 工具执行结果
    Note over App: 构造 tool 消息（role=tool）
    App->>LLM: 完整对话历史 + 工具结果
    LLM-->>App: 生成最终回复
```
...
<!-- ORIGINAL: LangGraph 工具编排流程图（START→agent→条件分支→tool_node/sandbox→循环回agent） -->
```mermaid
graph TD
    START(["START"]) --> agent["agent"]
    agent -->|"tool_calls?"| tool_node["tool_node<br/>（并行执行）"]
    agent -->|"sandboxed?"| sandbox["sandbox<br/>tool_node"]
    agent -->|"END"| END(["●"])
    tool_node -->|"工具结果注入 messages"| agent
    sandbox -->|"工具结果注入 messages"| agent
```
...
<!-- ORIGINAL: 屏幕理解流程（截屏→编码→视觉理解→操作决策→操作执行） -->
```mermaid
graph TD
    A["截屏捕获<br/>操作系统截屏 API 获取当前屏幕图像"] --> B["图像编码（Base64）<br/>缩放到合适分辨率以平衡精度与成本"]
    B --> C["视觉理解（VLM）<br/>识别 UI 元素 / 空间关系 / 文本内容 / 当前状态"]
    C --> D["操作决策<br/>结合用户目标和当前屏幕状态"]
    D --> E["操作执行<br/>调用操作系统 API 执行具体操作"]
```
...
<!-- ORIGINAL: 浏览器自动化 Agent 循环（页面理解→目标分解→操作执行→验证结果→循环） -->
```mermaid
graph TD
    Screenshot["截图 + DOM（可选）"] --> A
    A["页面理解<br/>(Perceive)"] -->|"当前状态"| B["目标分解<br/>(Plan)"]
    Goal["用户原始目标<br/>+ 已完成步骤"] --> B
    B -->|"下一步操作"| C["操作执行<br/>(Execute)"]
    C -->|"click / type / scroll"| D["验证结果<br/>(Verify)"]
    D -->|"目标未完成"| A
    D -->|"目标完成"| E["返回结果"]
```
...
<!-- ORIGINAL: 语义缓存工作流程（语义编码→相似度搜索→阈值判断→命中/未命中→执行+写入缓存） -->
```mermaid
graph TD
    Input["新的工具调用请求"] --> A["1. 语义编码（Embedding）<br/>将工具名 + 参数转为嵌入向量"]
    A --> B["2. 相似度搜索（Vector Search）<br/>在缓存中查找距离最近的向量"]
    B --> C{"3. 阈值判断"}
    C -->|"距离 < 阈值"| Hit["命中缓存<br/>返回缓存结果"]
    C -->|"距离 ≥ 阈值（未命中）"| D["4. 执行工具调用"]
    D --> E["5. 写入缓存"]
```
...
<!-- ORIGINAL: 去重决策逻辑（计算指纹→检查历史→未出现则执行/已出现则检查幂等性） -->
```mermaid
graph TD
    Input["新的工具调用: tool_name（args）"] --> A["计算调用指纹<br/>hash（tool_name + sorted args）"]
    A --> B{"该指纹是否在<br/>当前会话中出现过？"}
    B -->|"未出现过"| C["正常执行工具<br/>记录指纹和结果"]
    B -->|"已出现过"| D{"该工具是否为<br/>幂等操作？"}
    D -->|"是幂等"| E["返回缓存结果<br/>（跳过调用）"]
    D -->|"非幂等"| F["正常执行<br/>（副作用操作不可跳过）"]
```
...
<!-- ORIGINAL: 成本感知路由（用户提问→路由决策器→简单/中等/复杂查询分流） -->
```mermaid
graph TD
    Input["用户提问"] --> Router["路由决策器<br/>（低成本 LLM 或规则引擎）"]
    Router -->|"简单查询"| A["本地缓存<br/>成本: $0 / 延迟: 1ms"]
    Router -->|"中等查询"| B["向量检索<br/>成本: $ / 延迟: 50ms"]
    Router -->|"复杂查询"| C["联网搜索<br/>成本: $$ / 延迟: 2s"]
```

**级联调用策略**（Cascade）：先尝试低成本工具，如果结果不满意再升级到高成本工具：

<!-- ORIGINAL: 级联调用策略（语义缓存→知识库检索→外部搜索→高级LLM，逐级升级） -->
```mermaid
graph TD
    L1["Level 1: 语义缓存"] -->|"命中"| R1["返回结果（成本: $0）"]
    L1 -->|"未命中"| L2["Level 2: 本地知识库向量检索"]
    L2 -->|"置信度 > 0.85"| R2["返回结果（成本: $0.001）"]
    L2 -->|"置信度不足"| L3["Level 3: 调用外部搜索 API"]
    L3 -->|"找到结果"| R3["返回结果（成本: $0.01）"]
    L3 -->|"无满意结果"| L4["最终方案: 调用高级 LLM 综合推理"]
    L4 --> R4["返回结果（成本: $0.05）"]
```
...
<!-- ORIGINAL: Tool RAG 流程（离线：工具嵌入入库；在线：意图编码→向量检索→动态注入→LLM选择） -->
```mermaid
graph TD
    subgraph Offline["准备阶段（离线）"]
        O1["所有工具定义"] --> O2["为每个工具生成嵌入向量<br/>基于 name + description + 参数说明"]
        O2 --> O3["存入向量数据库"]
    end
    subgraph Online["运行阶段（在线）"]
        I["用户输入 / LLM 推理上下文"] --> A["1. 意图编码<br/>生成 query embedding"]
        A --> B["2. 向量检索<br/>查找最相关的 top_k 工具"]
        B --> C["3. 动态注入<br/>将 top_k 工具定义注入 Prompt"]
        C --> D["4. LLM 正常调用<br/>从少量候选工具中选择"]
    end
```
