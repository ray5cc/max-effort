# Max Effort — AI Agent 全栈开发知识库

> 覆盖 AI Agent 全链路开发知识（LLM 原理 → Prompt 工程 → 工具调用 → 记忆系统 → Agent 框架 → 后端服务 → 前端渲染 → 评测部署），同时提供前端、后端、数据库、算法、系统设计、DevOps 等核心领域的技术资料与面试指南。

## 项目简介

本项目分为两大根目录，内容分类一一对应，支持相互跳转：

| 目录 | 说明 |
|------|------|
| [01-技术资料](./01-技术资料/) | 各技术领域的系统性学习文档 |
| [02-面试指南](./02-面试指南/) | 对应领域的面试题目、解题思路与高频考点 |

## 技术分类

两大目录均按以下分类组织，分类编号一一对应：

1. **前端** — React/JavaScript/TypeScript/Next.js/状态管理/浏览器原理
2. **后端** — Java/Go/Python/Node.js/微服务/网络协议
3. **数据库** — Redis/PostgreSQL/MySQL/MongoDB/Elasticsearch/向量数据库
4. **算法与数据结构** — 排序搜索/动态规划/图算法/基础数据结构
5. **系统设计** — 分布式系统/消息队列/LLM推理引擎/分布式训练
6. **DevOps 与云计算** — Docker/Kubernetes/CI-CD/GitOps
7. **LLM 基础** — LLM原理/Transformer架构/Prompt工程/微调技术/PEFT/对齐训练/推理引擎
8. **AI Agent 全栈工程** — Agent架构/MCP协议/RAG工程化/对话系统/API网关/全链路设计
9. **编码题** — 编码技巧与解题策略、各领域手写代码实战

## 使用说明

- 每个目录下包含一个 `README.md` 总目录文件，列出所有子文档的链接与描述
- 文件夹和文件均以序号开头，保持有序
- 技术资料与面试指南的分类相互对应，方便对照学习与备考

## AI 工具接入

- [`AGENTS.md`](./AGENTS.md) — AI Agent 精简入口（TOC 式，Harness Engineering 规范）
- [`.agent-docs/operations.md`](./.agent-docs/operations.md) — 详细操作规范（渐进式披露）
- [`llms.txt`](./llms.txt) — 符合 llms.txt 规范，供 AI 工具快速理解项目结构
- [`CLAUDE.md`](./CLAUDE.md) — Claude AI 助手协作指南（含记忆系统最佳实践）

## 操作记录

变更日志存放于 [`CHANGELOG/`](./CHANGELOG/) 目录，按月份分文件记录。
